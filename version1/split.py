...
for i in range ( 0, CEt.size, 1) :
  if in_temp >= CEt[i] :
    ...
    lb_name = sys.argv[2] + "." + str(int(CEname[i])) + "c" # zzaaa.nnx format
...
if in_concentration == 0 :
  ...
  for i in range ( 0, SABt.size, 1) :
    if in_temp >= SABt[i] :
      ...
      lb_name = sys.argv[2] + "." + str(int(SABname[i])) + "t" # zzaaa.nnx format
...
# interpolate mCard for CE and SAB
