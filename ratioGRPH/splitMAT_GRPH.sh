...
function HELP {
  ...
  echo -e "Basic usage: $SCRIPT file.inp"\\n
  ...
  echo -e "Example: $SCRIPT file.inp"\\n
  exit 1
}
...
for (( inputcount=0; inputcount<$length; inputcount++ )) ; do
  ...
  flag=0
  if [[ -f ${case}_pseudoGRPH.inp ]]; then
    rm ${case}_pseudoGRPH.inp
  fi
  OLD_IFS=$IFS
  IFS=""
  while read -r line; do 
    ...
    if [[ ${array[0]} == [mM][0-9]* ]]; then
      ...
      python -O split_GRPH.py $in_temp ${array[1] /\.*c/} ${array[2]} $fileCE $fileSAB ${MAT}
      cat $MAT >> ${case}_pseudoGRPH.inp
      TinM=$in_temp
    ...
    ####################
      ...
      # argv[6]: new mCard file
      #                  argv[1]  argv[2]            argv[3]     argv[4] argv[5]  argv[6]
      python -O split_GRPH.py $TinM    ${array[1] /\.*t/} 0           $fileCE $fileSAB ${MAT}
      cat $MAT >> ${case}_pseudoGRPH.inp
    ####################
    # Condition 3
    ####################
...
#     echo "    --Condition 3."
      ...
      # argv[6]: new mCard file
      #                  argv[1]  argv[2]            argv[3]     argv[4] argv[5]  argv[6]
      python -O split_GRPH.py $in_temp ${array[0] /\.*c/} ${array[1]} $fileCE $fileSAB ${MAT}
      cat $MAT >> ${case}_pseudoGRPH.inp
      TinM=$in_temp
    ####################
    # Condition 4
    ####################
      ...
      IFS=""
      flag=0
      echo $line >> ${case}_pseudoGRPH.inp
    fi
    IFS=""
  done < $filename
  sed -i "s/0.0000E+00//g" ${case}_pseudoGRPH.inp
done
